...
const { isLoggedIn, validateKvpData, validateAttData, validateUserDataR  } = require("../middleware.js");
...
router.get('/registerMember',isLoggedIn, wrapAsync(authController.renderSignupForm));
router.post('/registerMember/saveUser',isLoggedIn,validateUserDataR, wrapAsync(authController.signup));
router.get('/registerMember/saveUser/successMsg',isLoggedIn, wrapAsync(authController.renderRegSuccessMsg));
...
router.post('/dashboardCRoom/regForm',isLoggedIn,validateKvpData, wrapAsync(userController.submitRegForm));
